...
const ASSET_VER = 'cards-v3'

// Respect Vite/Pages base URL for all public assets
...
function buildAllCardUrls(): string[] {
  ...
  return faces.map(u => `${asset(u)}?v=${ASSET_VER}`)
}
...
export default function App(){
  ...
  useEffect(() => {
    ...
    const imgs = g.hand.map(c => {
      const img = new Image()
      img.decoding = 'async'
      img.src = asset('cards/' + mapCardToFile(c.rank as unknown as string, c.suit as unknown as string))
      return img
    })
  ...
  useEffect(() => {
    ...
    if (g.result) {
      ...
      if (key !== lastWinRef.current) {
        lastWinRef.current = key
        if (g.result.payout > 0) {
                  ...
                  if (!c || !revealed) {
                    return (
                      <div key={i} className="card" style={{ borderRadius:8, overflow:'hidden', boxShadow:'2px 4px 8px rgba(0,0,0,0.25)' }}>
                        <img
                          src={`${asset('cards/back.svg')}?v=${ASSET_VER}`}
                          alt="Card back"
                          style={{ width:'100%', height:'100%', objectFit:'contain', display:'block' }}
                          width={240}
                          height={360}
                  ...
                  {g.hand.map((c, i) => {
                    ...
                    return (
                      <div key={c.id} className={`hintCard ${keep ? 'keep' : ''}`} title={`${c.rank} ${c.suit}`}>
                        <img src={asset(`cards/${fileName}`)} alt={`${c.rank} of ${c.suit}`} />
                      </div>
                    )
                  })}
                </div>
